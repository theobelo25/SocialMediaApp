...
// Unfollow a user
router.put('/follow/:userToUnfollowId', authMiddleware, async (req, res) => {
    const { userId } = req;
    ...
    try {
        ...
        const removeFollowing = user.following.map(
            following => following.user.toString()
        ).indexOf(userToUnfollowId);

        await user.following.splice(removeFollowing, 1);
        await user.save();

        const removeFollower = userToUnfollow.followers.map(
            follower => follower.user.toString()
        ).indexOf(userId)
...
router.post('/settings/password', authMiddleware, async (req, res) => {
    ...
    if (newPassword.length < 6) return res.status(401).send('Password must be at least 6 characters');
